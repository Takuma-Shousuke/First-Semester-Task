	...
	for( int i = 0; i < DIGITS; i++ )
	{
		...
		do 
		{
			...
			// 有効な値だったら isValid を true にする
				...
				{
					if (j == i)	// 値が同じではなく、見ているところが同じかどうか
							    // 同じ場所見てたら、同じ値は当たり前
					{
						continue;		// continue = それ以降を無視して、次の箱に行く
					}
					if (target[i] == target[j])
					{
						isValid = false;
						break;			// break = for文を抜ける(for文なんてなかったんだ。になる)
					}
					isValid = true;
				}
		...
		while( isValid == false );		// もし、ここが一致しなければ、whileを抜ける
										// もし、ここが一致すれば、doに戻る
	}
}
